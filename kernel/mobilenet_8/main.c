...
extern int8_t input_image[] __attribute__((aligned(32*NR_LANES)));
extern int8_t conv1_filter_3x3[] __attribute__((aligned(32*NR_LANES)));
extern int8_t pw_before_weights[] __attribute__((aligned(32*NR_LANES)));  
extern int8_t pw_before_bias[] __attribute__((aligned(32*NR_LANES)));     
extern int8_t dw_filters[] __attribute__((aligned(32*NR_LANES)));         
extern int8_t pw_after_weights[] __attribute__((aligned(32*NR_LANES)));   
extern int8_t pw_after_bias[] __attribute__((aligned(32*NR_LANES)));     
extern int8_t fc_weights[] __attribute__((aligned(32*NR_LANES)));         
extern int8_t fc_bias[] __attribute__((aligned(32*NR_LANES)));            
...
static void print_matrix_int(const int8_t *m, int H, int W, const char *name) {
    printf("\n--- %s (%dx%d) ---\n", name, H, W);
    for (int r = 0; r < H; r++) {
        for (int c = 0; c < W; c++)
            printf("%4d ", m[r*W + c]);
...
static void print_vector_int(const int8_t *v, int N, const char *name) {
    printf("\n--- %s (%d) ---\n", name, N);
    for (int i = 0; i < N; i++)
        printf("%d ", v[i]);
    printf("\n");
...
int main() {
    ...
    static int8_t pw1_out[PW_CH * PW1_OUT_H * PW1_OUT_W] __attribute__((aligned(32*NR_LANES)));
    static int8_t dw_out[PW_CH * DW_OUT_H * DW_OUT_W] __attribute__((aligned(32*NR_LANES)));
    static int8_t pw2_out[PW_CH * PW2_OUT_H * PW2_OUT_W] __attribute__((aligned(32*NR_LANES)));
    static int8_t pool_out[PW_CH * POOL_OUT_H * POOL_OUT_W] __attribute__((aligned(32*NR_LANES)));
