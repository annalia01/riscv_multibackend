...
int main() {
    ...
    static int8_t resblock_buf_a[RB_H * RB_W] __attribute__((aligned(32 * NR_LANES)));
    static int8_t resblock_buf_b[RB_H * RB_W] __attribute__((aligned(32 * NR_LANES)));
    static int8_t resblock_tmp[RB_H * RB_W] __attribute__((aligned(32 * NR_LANES)));
    static int8_t fc_out[FC_OUT] __attribute__((aligned(32 * NR_LANES)));

    const int8_t bn_mean   = 4;
    const int8_t bn_invstd = 1;
    const int8_t bn_gamma  = 1;
    const int8_t bn_beta   = 3;
    ...
    printf("\n[Layer] CONV1 7x7...\n");
    iconv2d_7x7_uint8(conv1_out, input_image, filter_7x7,
    ...
    int8_t *res_in  = resblock_buf_a;
    int8_t *res_out = resblock_buf_b;
    int dim_H=RB_H;
    int dim_W=RB_W;
    ...
    for (int b = 0; b < NUM_RES_BLOCKS; ++b) {
        ...
        iconv2d_3x3_uint8(resblock_tmp, res_in, filter_3x3,
                    dim_H, dim_W, 3);
